...
import { buildMonthlyHoursTrend, useTimeEntries } from "@/lib/time";
...
function TimeTrackingPage() {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    addEntry({ date, hours: parsedHours, note: note.trim() });
    setHours("");
    setNote("");
    setError("");
  }
    ...
    <AppShell>
      ...
      <Card className="my-5 p-5">
        ...
        <p className="mt-3 text-[12px] text-muted-foreground">
          Entries are saved to this browser only for now — a shared backend is coming later.
        </p>
      </Card>
          ...
          {sorted.map((entry) => (
            <div key={entry.id} className="flex items-start gap-3 px-6 py-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[13px] text-muted-foreground">{formatDate(entry.date)}</p>
              </div>
